...
@Component({
  selector: 'app-consultant-adm-register-gift-card',
...
export class RegisterGiftCardComponent implements OnInit {
  ...
  calcOff(evt) {
    const globalOff = this.valuePercentMin / 100;
  ...
  getCompany() {
    this.comPromise = Promise(resolve => {
      this._sharedService.getCompany().subscribe(
        (response: any) => {
          if (response.return) {
            this.objCompany = response.data.map(function (element) {
              return { id: element.companyForm.id, name: element.companyForm.name };
            });
            resolve(this.objCompany);
          } else {
            this._hyperToastsService.addToast('warn', 'Atenção', response.msg);
            this._router.navigate(['guide/register-company']);
          }
        },
  ...
  onSubmit() {
    ...
    if (this.registerForm.valid) {
      this._sharedService.postGiftCard(this.registerForm.value).subscribe(
        (response: any) => {
          if (response.return) {
            this._router.navigate(['/guide/edit-gift-card']);
          } else {
            this._hyperToastsService.addToast('warn', 'Atenção', response.msg);
          }
        },
