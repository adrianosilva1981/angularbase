...
@Component({
  selector: 'app-consultant-adm-withdraw',
...
export class WithdrawComponent implements OnInit {
      ...
      (response: any) => {
        if (response.return && response.data.length > 0 && response.activated) {
          ...
          if (response.activated) {
            ...
            this.msgs.push({
              severity: 'warn', summary: 'Prezado fornecedor, ', detail: 'habilite-se no <a href="https://office.youhub.com.br/">BackOffice</a> para então solicitar o resgate '
            });
          }
        }
      },
      ...
      response => {
        if (response.return && response.data != '') {
          ...
          const item = response.data.filter((product) => {
            if (product[0].id_buyShoppingOrder === selected.id) {
              return product[0];
            }
          });
  ...
  nextStep(calcTotal = false) {
    this.step++;
    this.dataSelectedTotalFormated = calcTotal ? Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(this.dataSelectedTotal - 10) : '0';
