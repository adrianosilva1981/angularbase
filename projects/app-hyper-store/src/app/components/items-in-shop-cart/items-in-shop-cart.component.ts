...
@Component({
  selector: 'app-hyper-store-items-in-shop-cart',
...
export class ItemsInShopCartComponent implements OnInit {
  ...
  constructor(
    ...
    BroadcastEventService.event('listenerLoginComponent').subscribe(evt => {
      if (evt.JWT) {
        ...
        this.resetVariables();
        this.init(this._hyperCookieService.getCookie_SHOPCART());
        this.attFieldPrice();
      }
    });
  }
  ...
  init(shopCart) {
    ...
    if (this.shopCartList) {
      ...
      const listGuide = this.shopCartList.filter(x => x.type === 'gift_code');
      ...
      const IDSGuide = listGuide ? Object.keys(listGuide).map(idx => listGuide[idx].id) : [];

      this.getRangeProducts(IDSProduct);
      this.getRangeServices(IDSServices);
      this.getRangeGuides(IDSGuide);
    } else {
      ...
      this._sharedService.listGiftId(filt).subscribe(
        (response: any) => {
          ...
          if (response.return) {
            const auxResponse = Array.isArray(response.data) ? response.data : [response.data];
            auxResponse.forEach(element => {
              const refItem = this.shopCartList.find(x => x.id == element.id && x.type === 'gift_code');
              if (refItem) {
                element.mediasCompanies = JSON.parse(element.mediasCompanies);
                element.photos = JSON.parse('[' + element.mediasCompanies.images + ']');
                element.typeShopCart = refItem.type;
                element.quantityShopCart = refItem.quantity;
                element.idShopCart = refItem.id;
