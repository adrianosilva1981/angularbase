...
@Component({
    selector: 'app-admin-youhubshop-info-products',
    templateUrl: './info-products.component.html',
    styleUrls: ['./info-products.component.less']
})
export class InfoProductsComponent implements OnInit {
    ...
    changeDate(interval: Date[]) {
        ...
        this.getInfoDetailsProducts(data);
    }

    getInfoDetailsProducts(data: { startDate: string; endDate: string; }) {
        this._sharedService.getInfoDetailsProducts(data).subscribe(
            response => {
                if (response.return) {
                    (this.products = response.data).forEach(giftCard => {
                        giftCard.buyers = JSON.parse(giftCard.buyers);
                        giftCard.status = giftCard.status === 'A' ? 'Ativo' : 'Inativo';
                        giftCard.hyper = giftCard.hyper === 'Y' ? 'Sim' : 'Não';
                        giftCard.spotlight = giftCard.spotlight === 'Y' ? 'Sim' : 'Não';
                    });
                } else {
