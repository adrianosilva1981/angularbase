...
@Component({
    selector: 'app-admin-back-office-info-graduation',
...
export class InfoGraduationComponent implements OnInit {
    ...
    public cols: any[] = [
        ...
        { field: 'graduation', header: 'Graduação' },
    ...
    exportExcel(dt: DataTable) {
        this._excelService.exportAsExcelFile((dt.filteredValue || dt.value), null, 'Graduação');
    }

    getSummaryGraduation() {
        const data = { startDate: new Date(this.interval[0]).toDateString(), endDate: new Date(this.interval[1]).toDateString() };
        this._sharedService.getSummaryGraduation(data).subscribe(
            response => {
                if (response.return) {
