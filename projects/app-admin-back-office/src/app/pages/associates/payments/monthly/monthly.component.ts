...
@Component({
    selector: 'app-admin-back-office-monthly',
...
export class MonthlyComponent implements OnInit {
    ...
    setAssociates(query: string = null) {
        this._sharedService.getMonthlyAssociates(query).subscribe(
            response => {
                if (response.return) {
                    this.cols = [];
                    this.associates = [];
    ...
    monthlyRequest(username: string) {
        this._router.navigate(['/associates/payments/monthly/' + username]);
    }
